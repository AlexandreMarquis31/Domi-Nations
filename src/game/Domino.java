...
public class Domino {
    ...
    private boolean sameTypeArounds(int x1, int y1, int x2, int y2, Board board) {
    ...
    private boolean sameTypePart(int x, int y, Board board, DominoPart part) {
        ...
        if (y - 1 > 0 && (board.get(x,y-1).type.equals(part.type) || board.get(x,y-1).type.equals("Chateau"))) {
            return true;
        }
        return false;
    }
    ...
    static boolean biggerThanBoard(int x1, int y1, int x2, int y2, Board board) {
